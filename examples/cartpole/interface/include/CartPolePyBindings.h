...
public:
   ...
   * CppAD library into.
   * @param [in] urdfFile: The absolute path to the URDF of the robot. This is
   * not used for cartpole.
   */
  CartPolePyBindings(const std::string &taskFile,
    ...
    // Robot interface
    CartPoleInterface cartPoleInterface(taskFile, libraryFolder,
                                        true /* verbose */);

    // MPC
    auto mpcPtr = std::make_unique<GaussNewtonDDP_MPC>(
        cartPoleInterface.mpcSettings(), cartPoleInterface.ddpSettings(),
    ...
    mpcPtr->getSolverPtr()->setReferenceManager(
        cartPoleInterface.getReferenceManagerPtr());
